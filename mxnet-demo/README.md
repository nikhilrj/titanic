...
# Remote debug on docker
Docker image used for this sample is based on L6 file - `mxnetdocker\Dockerfile.l6.cuda92`
Run `./run-docker-debug.sh` it will start container with mapped blobfuse drive volumme and code project volume
Container will start with Maven exec with Java remote debug settings. script is `startupDev.sh`

# Remote debug on AKS
Deploy application on kubernetes using mxnet-layer6:cuda92 image, code jar is copied to blobfuse and mapped to pod volume

Deploy app sample as k8s Job
```sbtshell
kubectl apply -f kube\pod.yml
```

Check pod has started by running `kubectl get pods` and getting logs from mxnet-demo pod to see that it started in listening mode
Port forward debug port to the pod, and start debugger by connection to localhost:5005
```sbtshell
kubectl port-forward mxnet-demo-txwg2  5005:5005
```


Blobfuse manifests are under `kube\` directory as well.
